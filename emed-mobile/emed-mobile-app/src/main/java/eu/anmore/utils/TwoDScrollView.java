...
public class TwoDScrollView extends FrameLayout {
	public TwoDScrollView(final Context context) {
		super(context);
		initTwoDScrollView();
	}
	public TwoDScrollView(final Context context, final AttributeSet attrs) {
		super(context, attrs);
		initTwoDScrollView();
	}

	public TwoDScrollView(final Context context, final AttributeSet attrs, final int defStyle) {
	...
	public boolean executeKeyEvent(final KeyEvent event) {
		...
		if (!canScroll()) {
			if (isFocused()) {
				...
				final View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, View.FOCUS_DOWN);
	 ...
	 */
	public boolean fullScroll(final int direction, final boolean horizontal) {
		if (!horizontal) {
	 ...
	 */
	public boolean arrowScroll(final int direction, final boolean horizontal) {
		View currentFocused = findFocus();
		if (currentFocused == this) {
			currentFocused = null;
		}
		...
		if (!horizontal) {
			if (nextFocused != null) {
				...
				final int scrollDelta = computeScrollDeltaToGetChildRectOnScreen(mTempRect);
	 ...
	 */
	@Override
	protected boolean onRequestFocusInDescendants(int direction, final Rect previouslyFocusedRect) {
	...
	private View findFocusableViewInMyBounds(final boolean topFocus, final int top, final boolean leftFocus,
			final int left, final View preferredFocusable) {
		/*
	 ...
	 *         found
	 */
	private View findFocusableViewInBounds(final boolean topFocus, final int top, final int bottom,
			final boolean leftFocus, final int left, final int right) {
	 ...
	 * @return true if the key event is consumed by this method, false otherwise
	 */
	private boolean scrollAndFocus(final int directionY, final int top, final int bottom, final int directionX,
	 ...
	 */
	private void scrollToChild(final View child) {
	 ...
	 */
	private boolean scrollToChildRect(final Rect rect, final boolean immediate) {
	 ...
	 * parent).
	 */
	private boolean isViewDescendantOf(final View child, final View parent) {
		if (child == parent) {
			return true;
		}
	...
	private int clamp(final int n, final int my, final int child) {
		if (my >= child || n < 0) {
			/*
